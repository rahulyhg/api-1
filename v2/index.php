...
function searchDeals($empid,$query,$page){
	...
	if (!ctype_alnum($query)){
		...
		return;
	 	...
	 	$q = "SELECT sql_calc_found_rows d.dealid, d.dealno, tcase(d.dealnm) as name, tcase(d.city) as city, tcase(d.area) as area, trim(concat(IFNULL(d.add1,''), ' ', IFNULL(d.add2,''), ' ', IFNULL(d.area,''), ' ', IFNULL(d.tahasil,''))) as address, round(fr.OdDueAmt) as overdue, round(fr.DueAmt) as total_due, round(fr.MinReqOD) as minreq_od, fr.rgid as bucket, d.Mobile as mobile, dg.mobile as guarantor_mobile, tcase(concat(dv.make, ' ', dv.model)) as vehicle_model, concat(fr.dd,'-',fr.mm,'-',fr.yy) as assigned_on, DATE_FORMAT(fr.CallerFollowupDt,'%d-%m-%Y') as caller_followup_dt,DATE_FORMAT(fr.SRAFollowupDt,'%d-%m-%Y') as sra_followup_dt
